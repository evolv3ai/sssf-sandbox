...
# The template ships apiKey "$OPENROUTER_API_KEY". pi only sees that variable
...
if [[ -n "$api_key" ]]; then
  models_json="${models_json//\$OPENROUTER_API_KEY/$api_key}"     # bash substitution, never argv
  say "runtime key baked in from .env"
else
  say "no OPENROUTER_API_KEY in .env — leaving the \$OPENROUTER_API_KEY placeholder (pi will need it exported)"
fi
